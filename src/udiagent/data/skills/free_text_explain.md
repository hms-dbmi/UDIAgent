...
Respond with a plain text answer using structured function references for any data-dependent values. **Keep the response to a single short paragraph** — accurate and helpful without preamble or padding. Do not include JSON, code blocks. Use markdown as appropriate to make responses easier to interpret with lists, bolding, etc. Do not us tick marks `` in markdown for inline-code.
